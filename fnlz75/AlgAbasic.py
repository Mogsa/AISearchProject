...
# ---- helper structures ----
# visibility matrix η_{ij} = 1/d_{ij}  (with small epsilon to avoid division by zero)
...
# initial pheromone τ0 (small positive constant)
...
# quick distance routine
def tour_length_of(path):
    length = 0
    for k in range(num_cities - 1):
        length += dist_matrix[path[k]][path[k + 1]]
...
for iteration in range(max_it):
    ...
    for ant in range(num_ants):
        ...
        while len(tour_tmp) < num_cities:
            ...
            if denom == 0.0:
                next_city = random.choice([c for c in range(num_cities) if not visited[c]])
            else:
                r = random.random()
                cumulative = 0.0
                next_city = probabilities[-1][0]  # fallback
        ...
        L = tour_length_of(tour_tmp)
        ants_tours.append(tour_tmp)
    ...
    for k, tour_k in enumerate(ants_tours):
        delta = Q / ants_lengths[k]
        for idx in range(num_cities - 1):
            i = tour_k[idx]
            j = tour_k[idx + 1]
            pheromone[i][j] += delta
            pheromone[j][i] += delta
        pheromone[tour_k[-1]][tour_k[0]] += delta
        pheromone[tour_k[0]][tour_k[-1]] += delta

    # ---- elitist reinforcement for best-so-far tour ----
    delta_best = elitist_weight * Q / best_length
    for idx in range(num_cities - 1):
        i = best_tour[idx]
        j = best_tour[idx + 1]
        pheromone[i][j] += delta_best
        pheromone[j][i] += delta_best
    pheromone[best_tour[-1]][best_tour[0]] += delta_best
    pheromone[best_tour[0]][best_tour[-1]] += delta_best
