...
import random
import math
# No longer using argparse for command-line arguments
...
def difference_velocity(a, b):
    ...
    for idx in range(num_cities):
        if temp[idx] != b[idx]:
            j = temp.index(b[idx])       # index where wanted city sits
            v.append((idx, j))
...
particles      = []  # current positions (tours)
velocities     = []  # current velocities
pbest          = []  # personal best tours
pbest_lengths  = []  # their lengths

for _ in range(num_parts):
    tour0 = list(range(num_cities))
    random.shuffle(tour0)
    vel0  = random_velocity()
    apply_velocity(tour0, vel0)  # first "flight"
    particles.append(tour0)
    velocities.append(vel0)
    pbest.append(tour0.copy())
    pbest_lengths.append(tour_length_of(tour0))

# global best
gbest_idx   = pbest_lengths.index(min(pbest_lengths))
gbest_tour  = pbest[gbest_idx].copy()
gbest_len   = pbest_lengths[gbest_idx]

# ---------------- main loop ----------------
for _ in range(max_iter):
    for idx in range(num_parts):
        cur_tour = particles[idx]
        cur_vel  = velocities[idx]

        # --- construct new velocity ---
        r1, r2 = random.random(), random.random()
        cognitive = multiply_velocity(
            difference_velocity(cur_tour, pbest[idx]), c1 * r1)
        social    = multiply_velocity(
            difference_velocity(cur_tour, gbest_tour), c2 * r2)
        inertia   = multiply_velocity(cur_vel, w_start)

        new_vel   = inertia + cognitive + social
        velocities[idx] = new_vel

        # --- move particle ---
        apply_velocity(cur_tour, new_vel)

        # --- evaluate & update personal best ---
        cur_len = tour_length_of(cur_tour)
        if cur_len < pbest_lengths[idx]:
            pbest[idx]         = cur_tour.copy()
            pbest_lengths[idx] = cur_len

            # --- update global best if needed ---
            if cur_len < gbest_len:
                gbest_len  = cur_len
                gbest_tour = cur_tour.copy()

# ---------------- results ----------------
tour        = gbest_tour
tour_length = gbest_len
